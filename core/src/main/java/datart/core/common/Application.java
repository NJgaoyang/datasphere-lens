...
public class Application implements ApplicationContextAware {
    ...
    private static String generatedTokenSecret;

    public static String getTokenSecret() {
        String secret = getProperty("datart.security.token.secret");
        if (StringUtils.isNotBlank(secret)) {
            return secret;
        }
        // Generate and cache a random secret at startup when not configured
        // This prevents the use of the well-known hardcoded default key
        if (generatedTokenSecret == null) {
            generatedTokenSecret = UUIDGenerator.generate() + UUIDGenerator.generate();
            log.warn("================================================");
            log.warn("Security warning: 'datart.security.token.secret' is not configured.");
            log.warn("A random secret has been generated for this session, but it will");
            log.warn("change on restart. Please set a persistent secret in your configuration:");
            log.warn("  datart.security.token.secret=<your-strong-secret>");
            log.warn("================================================");
        }
