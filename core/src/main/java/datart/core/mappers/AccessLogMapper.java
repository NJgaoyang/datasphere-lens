...
public interface AccessLogMapper extends CRUDMapper {
    ...
    @Insert({
        ...
        "#{resourceType,jdbcType=VARCHAR}, #{resourceId,jdbcType=VARCHAR}, #{resourceName,jdbcType=VARCHAR}, ",
    ...
    @Select({
        "select",
        "id, `user`, resource_type, resource_id, resource_name, access_type, access_time, duration",
        "from access_log",
        "where id = #{id,jdbcType=VARCHAR}"
    })
    @Results({
