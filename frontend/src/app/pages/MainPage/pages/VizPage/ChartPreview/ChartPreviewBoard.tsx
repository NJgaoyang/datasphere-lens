...
const ChartPreviewBoard: FC<{
  ...
  ({
    ...
    useEffect(() => {
      ...
      )?.filters as PendingChartDataRequestFilter[];
      const jumpVariableParams: Record<string, any[]> = parse(
        filterSearchUrl || '',
        {
          ignoreQueryPrefix: true,
        },
    ...
    const handleAddToDashBoard = useCallback(
      (dashboardId, dashboardType) => {
        ...
        try {
          navigate(
            `/organizations/${orgId}/vizs/${dashboardId}/boardEditor`,
            {
              state: {
    ...
    const redirect = useCallback(
      tabKey => {
        if (tabKey) {
          navigate(`/organizations/${orgId}/vizs/${tabKey}`);
        } else {
