  ...
  extraReducers: builder => {
    ...
    builder.addCase(getLoggedInUserPermissions.fulfilled, (state, action) => {
      ...
      Object.values(ResourceTypes).forEach(type => {
        ...
        permissionMap[type] = privilegeMap;
        // 预计算模块级访问标志: 有 '*' 权限或任意资源级权限
        moduleAccessMap[type] = Object.keys(privilegeMap).some(
          k => privilegeMap[k] > 0,
        );
      });
      state.permissionMap = permissionMap;
      state.moduleAccessMap = moduleAccessMap;
    });
