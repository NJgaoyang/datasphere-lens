...
public class ShiroSecurityManager implements DatartSecurityManager {
    ...
    public boolean validateUser(String username, String password) throws AuthException {
        ...
        try {
            ...
            log.warn("Rejected login for user {} because the stored password is not BCrypt encoded", username);
            return false;
        }
    }
    ...
    private final ThreadLocal<Subject> originalSubjectHolder = new ThreadLocal<>();

    @Override
    public void runAs(String userNameOrEmail) {
        originalSubjectHolder.set(SecurityUtils.getSubject());
    ...
    public void releaseRunAs() {
        logoutCurrent();
        Subject originalSubject = originalSubjectHolder.get();
        originalSubjectHolder.remove();
