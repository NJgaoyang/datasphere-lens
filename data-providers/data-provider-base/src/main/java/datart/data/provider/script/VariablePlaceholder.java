...
public class VariablePlaceholder {
    ...
    private ReplacementPair replacePermissionVariable(List<ScriptVariable> variables) {
        ...
        if (CollectionUtils.isEmpty(variables)) {
            // Permission checks must fail closed. A missing variable usually means
            // that the current user has no value configured for this permission.
