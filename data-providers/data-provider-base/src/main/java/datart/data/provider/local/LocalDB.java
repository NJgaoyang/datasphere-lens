...
public class LocalDB {
    ...
    private static final String SET_EXPIRE_SQL = "INSERT INTO `cache_expire` VALUES( ?, PARSEDATETIME(?,?)) ";

    private static final String DELETE_EXPIRE_SQL = "DELETE FROM `cache_expire` WHERE `source_id`=? ";
    ...
    public static boolean checkCacheExpired(String cacheKey) throws SQLException {
        try (Connection connection = getConnection(true, null);
             PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM `cache_expire` WHERE `source_id`=?")) {
            preparedStatement.setString(1, cacheKey);
    ...
    private static void setCacheExpire(String sourceId, java.util.Date date) throws SQLException {
        try (Connection connection = getConnection(true, null)) {
            // delete first
            try (PreparedStatement deleteStatement = connection.prepareStatement(DELETE_EXPIRE_SQL)) {
                deleteStatement.setString(1, sourceId);
                deleteStatement.execute();
            }
            // insert expire
            try (PreparedStatement insertStatement = connection.prepareStatement(SET_EXPIRE_SQL)) {
                insertStatement.setString(1, sourceId);
                insertStatement.setString(2, DateFormatUtils.format(date, Const.DEFAULT_DATE_FORMAT));
                insertStatement.setString(3, Const.DEFAULT_DATE_FORMAT);
    ...
    public static void clearCache(String cacheKey) throws SQLException {
        try (Connection connection = getConnection(true, null);
             PreparedStatement preparedStatement = connection.prepareStatement(DELETE_EXPIRE_SQL)) {
            preparedStatement.setString(1, cacheKey);
