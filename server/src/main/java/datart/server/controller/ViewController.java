...
public class ViewController extends BaseController {
    ...
    @Operation(summary = "compose two views into a database-pushdown SQL view")
    @PostMapping("/compose")
    public ResponseData<ViewComposeResult> compose(@Validated @RequestBody ViewComposeParam param) throws Exception {
        return ResponseData.success(viewService.compose(param));
    }

    @Operation(summary = "get view lineage")
    @GetMapping("/{viewId}/lineage")
    public ResponseData<ViewLineageDTO> getLineage(@PathVariable String viewId) {
        checkBlank(viewId, "viewId");
        return ResponseData.success(viewService.getLineage(viewId));
    }
