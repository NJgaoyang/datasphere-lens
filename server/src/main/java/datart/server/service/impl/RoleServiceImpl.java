...
public class RoleServiceImpl extends BaseService implements RoleService {
    ...
    public RelRoleResource upsertPermission(RelRoleResource relRoleResource) {
        ...
        if (StringUtils.isBlank(relRoleResource.getId())) {
            // 先根据唯一键(role_id, resource_id, resource_type)查询是否已存在记录
            RelRoleResource existing = rrrMapper.selectByUniqueKey(
                    relRoleResource.getRoleId(),
                    relRoleResource.getResourceId(),
                    relRoleResource.getResourceType());
            if (existing != null) {
                // 已存在则更新
                existing.setPermission(relRoleResource.getPermission());
                existing.setUpdateTime(new Date());
                existing.setUpdateBy(getCurrentUser().getId());
                rrrMapper.updateByPrimaryKeySelective(existing);
                return existing;
            }
            // 不存在则插入
    ...
    public SubjectPermissions getSubjectPermissions(String orgId, SubjectType subjectType, String subjectId) {
        ...
        // 权限审计: 逐个检查每个模块的权限状态。
        // 若用户无某模块的模块级权限（'*'）且无该模块的任何资源级权限，则隐藏该模块。
        // 数据级联依赖 (SOURCE → VIEW → VIZ) 在数据执行层强制检查，不通过隐式授权。
        if (!permissions.isOrgOwner()) {
            Set<String> hideInNav = new HashSet<>();

            // 需要检查的导航模块（ROLE 与 USER 共用"成员"导航，需一并检查）
            String[] navModules = {"VIZ", "VIEW", "SOURCE", "SCHEDULE", "USER", "ROLE", "MANAGER"};

            for (String module : navModules) {
                // USER 和 ROLE 共用"成员"导航，任一有权限则都不隐藏
                if ("ROLE".equals(module)) {
                    continue; // 单独处理
                }
                List<RelRoleResource> moduleResources = groupedResource.get(module);

                // 对于 USER 模块，同时检查 ROLE 模块的权限
                if ("USER".equals(module)) {
                    List<RelRoleResource> roleResources = groupedResource.get("ROLE");
                    boolean hasModulePermission = (moduleResources != null &&
                            moduleResources.stream().anyMatch(r -> "*".equals(r.getResourceId())))
                            || (roleResources != null &&
                            roleResources.stream().anyMatch(r -> "*".equals(r.getResourceId())));
                    // 模块级权限('*')缺失时隐藏模块
                    if (!hasModulePermission) {
                        hideInNav.add(module);
                        hideInNav.add("ROLE");
                    }
                    continue;
                }
                ...
                // 检查是否有模块级权限（'*'）
                // 注意: '*' 被 Disable 时 upsertPermission 会删除该记录，所以 '*' 不存在也视为 Disable
                boolean hasModulePermission = moduleResources != null &&
                        moduleResources.stream().anyMatch(r -> "*".equals(r.getResourceId()));

                // 模块级权限('*')缺失时隐藏模块（即使有资源级权限）
                // 资源级权限仅用于数据访问控制，不影响模块导航可见性
                if (!hasModulePermission) {
                    hideInNav.add(module);
                }
            }
            ...
            if (!hideInNav.isEmpty()) {
                permissions.setHideInNav(hideInNav);
            }
        }
