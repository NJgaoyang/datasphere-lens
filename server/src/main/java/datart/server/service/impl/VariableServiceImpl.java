...
public class VariableServiceImpl extends BaseService implements VariableService {
    ...
    public void requirePermission(Variable entity, int permission) {
        if (entity.getViewId() != null) {
            View view = retrieve(entity.getViewId(), View.class, false);
            Application.getBean(ViewService.class).requirePermission(
                    view, permission == Const.READ ? Const.READ : Const.MANAGE);
            return;
        }
        if (entity.getSourceId() != null) {
            Source source = retrieve(entity.getSourceId(), Source.class, false);
            Application.getBean(SourceService.class).requirePermission(
                    source, permission == Const.READ ? Const.READ : Const.MANAGE);
            return;
        }
        securityManager.requireOrgOwner(entity.getOrgId());
    }
    ...
    public Variable create(BaseCreateParam createParam) {
        ...
        Variable permissionTarget = new Variable();
        BeanUtils.copyProperties(variableCreateParam, permissionTarget);
        requirePermission(permissionTarget, Const.CREATE);
        checkUnique(variableCreateParam.getOrgId(), variableCreateParam.getViewId(), variableCreateParam.getName());
    ...
    public List<RelVariableSubject> listViewVariableRels(String viewId) {
        requireViewPermission(viewId, Const.READ);
    ...
    public List<RelVariableSubject> listVariableValues(String variableId) {
        requirePermission(retrieve(variableId, false), Const.MANAGE);
        return rvsMapper.selectVariableRels(variableId);
    }

    @Override
    public List<VariableValue> listOrgValue(String orgId) {
        // Organization owners still need permission variables to be present in
        // the rendered SQL. DataProviderService marks them as disabled so their
        // predicates become 1=1. Dropping them here would make the renderer
        // treat them as unknown and fail closed with 1=0.
        List<Variable> variables = variableMapper.selectOrgVariables(orgId);
        return convertVariables(variables);
    }
    ...
    public boolean deleteByIds(Set<String> variableIds) {
        if (!CollectionUtils.isEmpty(variableIds)) {
            variableIds.forEach(id -> requirePermission(retrieve(id, false), Const.MANAGE));
        }
    ...
    public RelVariableSubject createRel(RelVariableSubject relVariableSubject) {
        requirePermission(retrieve(relVariableSubject.getVariableId(), false), Const.MANAGE);
    ...
    private boolean updateRel(RelVariableSubject relVariableSubject) {
        RelVariableSubject stored = rvsMapper.selectByPrimaryKey(relVariableSubject.getId());
        if (stored == null) {
            Exceptions.msg("Variable relation not found: " + relVariableSubject.getId());
        }
        requirePermission(retrieve(stored.getVariableId(), false), Const.MANAGE);
    ...
    public boolean deleteRel(Set<String> relIds) {
        if (!CollectionUtils.isEmpty(relIds)) {
            relIds.forEach(id -> {
                RelVariableSubject rel = rvsMapper.selectByPrimaryKey(id);
                if (rel == null) {
                    Exceptions.msg("Variable relation not found: " + id);
                }
                requirePermission(retrieve(rel.getVariableId(), false), Const.MANAGE);
            });
        }
    ...
    public boolean delViewVariables(String viewId) {
        requireViewPermission(viewId, Const.MANAGE);
    ...
    private List<VariableValue> convertVariables(List<Variable> variables) {
        ...
        return variables.stream()
                // Keep permission variables even when the current user/role has no
                // configured value. The SQL renderer will turn an empty permission
                // value into 1=0. Dropping the variable here makes the renderer
                // unable to distinguish "no permission" from an unknown variable.
                .filter(var -> VariableTypeEnum.PERMISSION.name().equals(var.getType())
                        || VariableTypeEnum.QUERY.name().equals(var.getType()))
                .map(var -> {
    ...
    private void requireViewPermission(String viewId, int permission) {
        View view = retrieve(viewId, View.class, false);
        Application.getBean(ViewService.class).requirePermission(view, permission);
    }
